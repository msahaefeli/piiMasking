...
- チャンク化: 大きなトランスクリプトは Text Analytics の制限に合わせて自動的にチャンク分割します。可能な限り文節（句点・改行）で切り、境界でのエンティティ分割を低減します。
- オーバーラップ: チャンク間でエンティティが跨るケースを検出するために重複領域を持たせます。重複は後で集約・除去されます。
- マージポリシー: チャンクから収集した検出をオフセットに基づいて統合し、重複や重なりを解消します。カテゴリが異なる場合は `カテゴリA/カテゴリB` のように連結するルールを用いています。
- 設定可能項目（環境変数）: `PII_RETRY_MAX_COUNT`, `PII_RETRY_BASE_DELAY_MS`, `PII_RETRY_MAX_DELAY_MS`, `PII_CHUNK_MAX_SIZE`, `PII_CHUNK_OVERLAP`
