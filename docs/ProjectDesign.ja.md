...
注: デフォルトでは、法人向け保険営業で CRM に保存しないことが想定される高感度識別子のみをマスクします（例: クレジットカード番号、国民識別番号、銀行口座番号、パスポート、運転免許証、IP アドレス、URL 等）。`Person`、`PhoneNumber`、`Email`、`Address` といった連絡先フィールドは CRM ワークフローを支援するためデフォルトでマスクされません。動作を変更するには `PII_MASK_CATEGORIES` を設定してください。
